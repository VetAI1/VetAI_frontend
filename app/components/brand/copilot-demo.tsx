          ...
          <div className="min-w-0 flex-1">
            ...
            {!typing && !analyzing && visible > 0 && (
              <div className="animate-in space-y-2 fade-in duration-200">
                ...
                {FINDINGS.slice(0, visible).map((finding, index) => (
                  <div
                    key={finding.label}
                    className="animate-in flex items-center justify-between gap-3 rounded-xl border border-stone-200 dark:border-stone-800 bg-[oklch(0.985_0.01_95)] dark:bg-stone-950 px-3.5 py-2.5 fade-in duration-200 [animation-timing-function:var(--ease-brand)]"
                ...
                {visible >= FINDINGS.length && (
                  <p
                    className="animate-in flex items-center gap-1.5 pt-1 text-xs font-semibold text-teal-800 dark:text-teal-500 fade-in duration-200"
                    style={{ animationDelay: `${FINDINGS.length * 60}ms` }}
                  >
                    <Sparkles size={13} />
